...
class StaticFileMiddleware(BaseMiddleware):
    ...
    def get_content_type(self, path: Path) -> str | None:
    ...
    def find_static_file(self, path: Path) -> Path:
        ...
        found_files: list[Path] = list(self.static_dir.glob(f"*/{path}"))
    ...
    def send_file(self, environ, start_response, file_path, mime_type):
        mod_time: str = datetime.fromtimestamp(file_path.stat().st_mtime).strftime(
            "%a, %d %b %Y %H:%M:%S GMT"
        )
    ...
    def __call__(self, environ, start_response):
        ...
        file_path: Path = self.find_static_file(path)
        mime_type: str | None = self.get_content_type(path)

        if not file_path.is_file():
            return self.app(environ, start_response)
        elif mime_type is None:
            return self.send_error(
                start_response, HttpStatus.BAD_REQUEST, BaseController.headers
            )
        else:
